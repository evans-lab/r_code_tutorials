#Lifted from http://jonlefcheck.net/2012/10/24/nmds-tutorial-in-r/ 
...
set.seed(2)#integer vector, containing the random number generator
community_matrix=matrix(sample(1:100,300,replace=T),nrow=10, #random number draw sample(x, size, replace = FALSE, prob = NULL)
                        dimnames=list(paste("community",1:10,sep=""),
                                      paste("sp",1:30,sep="")))#labeling a species matrix with 10 communities (rows) and 30 species (columns)
...
elevation=runif(10,0.5,1.5)#enerates random deviates 
